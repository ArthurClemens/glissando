...
__webpack_require__.r(__webpack_exports__);
/* harmony export */ __webpack_require__.d(__webpack_exports__, {
/* harmony export */   "createLocationsFromPath": function() { return /* reexport safe */ glissando__WEBPACK_IMPORTED_MODULE_0__.createLocationsFromPath; },
...
/**
 * Create an object with keys for path depth so that it can be used for merging.
 */
const indexTrail = trail =>
  trail.reduce(
    (acc, path, index) => ({
      ...acc,
      [index.toString()]: path,
...
/**
 * From a path create a trail - a list of path fragments.
 * Pathname:
...
const createTrail = path => {
  ...
  return path
    ...
    .reduce((acc, fragment) => {
      ...
      const fpath = previousPath ? previousPath + fragment : fragment;
      return [...acc, fpath];
    }, []);
};
/**
 * The breadcrumb may have incomplete data, because when traversing backward,
 * the model locations also contain the locations already visited.
 * Here we combine the location list with the breadcrumb list,
 * excluding deeper locations that do not match the parent path.
 */
const consolidatePaths = (trail = [], modelLocations = []) => {
  const merged = {
    ...indexTrail(modelLocations),
    ...indexTrail(trail),
  };
  console.log('indexTrail(modelLocations)', indexTrail(modelLocations));
  console.log('indexTrail(trail)', indexTrail(trail));
  console.log('merged', merged);
  const values = Object.keys(merged).map(key => merged[key]);
  const pathCounts = values.reduce((acc, path) => {
    const lengthKey = path.split('/').length.toString();
    return {
      ...acc,
      [lengthKey]: true,
    };
  }, {});
  const isHierarchical = Object.keys(pathCounts).length > 1;
  if (!isHierarchical) {
    return {
  ...
  const locations = values.reduce((acc, path) => {
    const previousPath = acc[acc.length - 1] || '';
    if (previousPath && !new RegExp(`^${previousPath}`).test(path)) {
      return acc;
    }
    return [...acc, path];
  }, []);
 ...
 * );
 */
const createLocationsFromPath = (path = '', modelLocations = []) => {
  /*
    Starting from any (deeper) location, where we maybe don't have locations stored
    in the Glissando model yet. A breadcrumb list the pages that should come before.
    */
  const trail = createTrail(path);
  /*
    Combine model locations with the breadcrumb.
    Exclude invalid paths.
    */
  const { locations, isHierarchical } = consolidatePaths(trail, modelLocations);
  console.log('isHierarchical', isHierarchical);
  const location = isHierarchical ? locations[trail.length - 1] : path;
  return {
    locations,
    location,
  };
